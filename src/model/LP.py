...
class LinkPredictionModel(nn.Module):
    ...
    def forward(self, samples):
        ...
        x = graph.x.to(device0)                          # [total_nodes, hidden_dim]
        ...
        q_expand = torch.cat(q_expand_list, dim=0).to(device0)
        q_emb = torch.cat(q_emb_list, dim=0).to(device0)
            
        candidate_edge_index = torch.cat(c_edge_index_list, dim=1).to(device0)  # [2, total_E]
        candidate_edge_attr = torch.cat(c_edge_attr_list, dim=0).to(device0) 
        src_list = torch.stack(src_list, dim=0).long().to(device0)
        dst_list = torch.stack(dst_list, dim=0).long().to(device0)
        ...
        # 将 src, dst list 变成张量
        src_list = torch.stack(samples['src'], dim=0).long().to(device0)  # [B]
        dst_list = torch.stack(samples['dst'], dim=0).long().to(device0)  # [B]
        labels = torch.stack(samples['lp_label'], dim=0).float().to(device0)  # [B]
        ...
        # candidate_edge_index = samples['candidate_edge_index'].to(device0)
        # candidate_edge_attr = samples['candidate_edge_attr'].to(device0)
    ...
    def inference(self, samples):
        graph = samples['graph']
        x = graph.x.to(device0)
        edge_index = graph.edge_index
        edge_attr = graph.edge_attr # [num_edges, hidden_dim]
        q_emb = graph.question_node[:1].to(device0) #[1, hidden_dim] 

...
def get_augmented_triplets(path_model, path_tokenizer, subgraph_edge_indices, textual_nodes, textual_edges,
                           edges_mapping):
    node_texts = textual_nodes["node_attr"].to_numpy()
    edge_texts = textual_edges["edge_attr"].to_numpy()
    if subgraph_edge_indices[0] == subgraph_edge_indices[1]:
...
def find_topk_subgraph(graph, q_emb, top_k_indices, edges_mapping, textual_nodes, textual_edges, k, topk_entity):
    ...
    for node_id in top_k_indices:
        try:
            ...
            subgraph = Data(x=graph.x[node_id].unsqueeze(0), edge_index=torch.empty((2, 0), dtype=torch.long),
                            edge_attr=None)
        ...
        subnodes_desc = textual_nodes.iloc[subgraph_nodes]
        ...
        if subgraph.edge_attr is None:
            ...
            subgraph_edge_indices = [edges_mapping.index((src, dst)) for _, (src, dst) in
                                     enumerate(subgraph.edge_index.T.numpy()) if (src, dst) in edges_mapping]
            # subedges_desc = textual_edges.iloc[subgraph_edge_indices].iloc[top_k_edges]
            subedges_desc = textual_edges.iloc[subgraph_edge_indices]
            topk_desc += subnodes_desc.to_csv(index=False) + '\n' + subedges_desc.to_csv(index=False,
                                                                                         columns=['src', 'edge_attr',
                                                                                                  'dst']) + '\n'
...
def retrive_on_graphs(graph, q_emb, textual_nodes, textual_edges, topk=10, k=2, topk_entity=5, augment="none",
                      sims=None):
    ...
    if sims is not None:
        ...
        return find_topk_subgraph(graph, q_emb, top_k_indices, edges_mapping, textual_nodes, textual_edges, k,
                                  topk_entity)
    ...
    for node_id in range(graph.num_nodes):
        ...
        try:
            ...
            subgraph = Data(x=graph.x[node_id].unsqueeze(0), edge_index=torch.tensor([], dtype=torch.long).view(2, -1),
                            edge_attr=None)
        ...
        if augment == "path":
            ...
            flatten_graph = get_augmented_path(path_model, path_tokenizer, paths, textual_nodes, textual_edges,
                                               edges_mapping)
            flatten_graphs.append(flatten_graph)
        elif augment == "triplet":
            flatten_graph = get_augmented_triplets(path_model, path_tokenizer, subgraph_edge_indices, textual_nodes,
                                                   textual_edges, edges_mapping)
            flatten_graphs.append(flatten_graph)
        elif augment == "trunk":
    ...
    return sims, find_topk_subgraph(graph, q_emb, top_k_indices, edges_mapping, textual_nodes, textual_edges, k,
                                    topk_entity)

...
class CompletionModel(torch.nn.Module):
    ...
    def forward(self, samples, return_attr=False):
        ...
        # print(f'label:{samples["completion_label"]}')
        ...
        bos_embeds = self.word_embedding(
            self.tokenizer(BOS, add_special_tokens=False, return_tensors='pt').input_ids[0].to(embedding_device))
        pad_embeds = self.word_embedding(torch.tensor(self.tokenizer.pad_token_id).to(embedding_device)).unsqueeze(0)

        # encode graphs
        graph_embeds = self.encode_graphs(samples, 'train')
        # graph_embeds = self.encode_graphs(samples, 'train').to(embedding_device)
        graph_embeds = self.projector(graph_embeds)
        graph_embeds = graph_embeds.to(embedding_device)
        ...
        for i in range(batch_size):
            ...
            embedding_device = self.model.get_input_embeddings().weight.device
            inputs_embeds = self.word_embedding(torch.tensor(input_ids).to(embedding_device))
        ...
        inputs_embeds = torch.stack(batch_inputs_embeds, dim=0).to(embedding_device)
        attention_mask = torch.tensor(batch_attention_mask).to(embedding_device)
        label_input_ids = torch.tensor(batch_label_input_ids).to(embedding_device)
    ...
    def inference(self, sample):
        ...
        attention_mask = torch.tensor(attention_mask).to(embedding_device)

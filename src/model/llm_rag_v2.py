...
class GraphLLM(torch.nn.Module):
    ...
    def __init__(
        ...
        **kwargs
        ...
        self.tokenizer = AutoTokenizer.from_pretrained(
            args.llm_model_path,
            trust_remote_code=True  # 使用LLaMA3推荐的chat_template
        )
        if self.tokenizer.pad_token_id is None:
            self.tokenizer.pad_token_id = self.tokenizer.eos_token_id  # LLaMA3默认没有pad token
    ...
    def encode_graphs(self, sample):
        graph = sample['graph']
        graph = graph.to(self.model.device)
        # print("graph.x:", graph.x.shape)
        # print("##graph.edge_index:", graph.edge_index.shape, "\n", graph.edge_index.long())
        # print("graph.question_node:", graph.question_node.shape)
        # print("graph.edge_attr:", graph.edge_attr.shape)
        # print("##graph.question_edge:", graph.question_edge.shape)
    ...
    def inference(self, samples):
        ...
        batch_input_ids = []
        for q, d in zip(samples["question"], samples["desc"]):
            messages = [{"role": "user", "content": f"{d}\n{q}"}]
            input_ids = self.tokenizer.apply_chat_template(messages, return_tensors="pt", add_generation_prompt=True).squeeze(0)
            batch_input_ids.append(input_ids.to(self.model.device))
        ...
        inputs_embeds = self.word_embedding(input_ids_padded)

        # 添加 graph embedding
        batch_size = len(samples['id'])
        for i in range(batch_size):
            inputs_embeds[i, 0:1] = torch.cat([graph_embeds[i].unsqueeze(0)], dim=0)  # 替换首 token embedding（可调整位置）
        ...
        # inputs_embeds = self.word_embedding(torch.tensor(input_ids).to(self.model.device))
        # inputs_embeds = torch.cat([bos_embeds, inputs_embeds], dim=0) # 后面看看这里维度拼接怎么样
        ...
        # return {'graph_id': sample['graph_id'],

...
def get_accuracy_expla_graphs_lp(eval_output, path):
    converted_output = []
    for entry in eval_output:
        # print(f'******labeltype:{entry["label"]}')
        label = torch.cat(entry["label"])
        converted_output.append({
            "pred": entry["pred"].tolist(), 
            "label": label.tolist(), 
        })

    df = pd.DataFrame(converted_output)

    with open(path, "w") as f:
        for _, row in df.iterrows():
            pred_str = json.dumps(row["pred"])
            label_str = json.dumps(row["label"])
            json_line = f'{{"pred": {pred_str},\n"label": {label_str}}}\n'
            f.write(json_line)
            
    total_acc = 0.0
    for entry in eval_output:
        pred = entry["pred"].float()
        label = torch.cat(entry["label"]).float()
        match = (pred == label).float().mean() 
        total_acc += match.item()

    avg_acc = total_acc / len(eval_output)
    return avg_acc
...
eval_funcs = {
    ...
    # "expla_graphs": get_accuracy_expla_graphs_lp,
